...
@objc public class Request:NSObject {
    ...
    public static func parametersPool() -> [String: Any] {
        if osString.isEmpty {
        ...
        return ["manufacturer": "apple",
                ...
                "network_type": "wifi",
                "android_id": "",
                "distinct_id": CacheUtil.shared.getUUID(),
                "build": "Build/\(Bundle.main.infoDictionary?["CFBundleVersion"] ?? "1")",
                "referrer_url": "",
                "install_version": "",
                "user_agent":  CacheUtil.shared.getUserAgent(),
...
//                "ad_sense": "",
//                "ad_format": ad?.position.rawValue ?? "",
...
//                "ad_sdk_ver": ""
    ...
    @objc static func objcTbaRequest(key:String) {
        if key == "install" {
            self.tbaRequest(key: .install)
        }
        if key == "first_Open" {
            self.tbaRequest(key: .firstOpen)
        }
    }
...
extension Request {
    private func startRequest() -> Void {
        ...
        // 重新赋值 log_id
        parameters = deepModifyLogID(in: Request.commonParam, logKeyName: Request.logKeyName, dateKeyNames: Request.dateKeyNames)
        ...
        dataRequest.responseData { (result: AFDataResponse) in
            guard let code = result.response?.statusCode, code == RequestCode.success.rawValue else {
                ...
                NSLog("[API] ❌❌❌ key:\(self.key.rawValue) code: \(code) error:\(retStr ?? "")")
                self.handleError(code: code, error: retStr, request: result.request)
                return
            }
            if let data = result.data {
                let retStr = String(data: data, encoding: .utf8) ?? ""
                NSLog("[API] ✅✅✅ key: \(self.key.rawValue) response \(retStr)")
                self.requestSuccess(retStr)
            } else {
                NSLog("[API] ❌❌❌ event: \(self.key.rawValue) response data is nil")
    ...
    func deepModifyLogID(in dictionary: [String: Any], logKeyName: String, dateKeyNames: [String]) -> [String: Any] {
        var modifiedDict = dictionary
        
        for (key, value) in dictionary {
            if let nestedDict = value as? [String: Any] {
                modifiedDict[key] = deepModifyLogID(in: nestedDict, logKeyName: logKeyName, dateKeyNames: dateKeyNames)
            } else if key == logKeyName {
    ...
    func handleError(code:Int, error: Any?, request: URLRequest?) -> Void {
        // 通过id进行缓存
        if key.isFirstOpen {
            if CacheUtil.shared.cacheOfFirstOpenFailCount() < 5 {
                CacheUtil.shared.appendCache(RequestCache(id, key: key, eventKey: eventKey, req: request))
                if CacheUtil.shared.cache(id) == nil {
                    CacheUtil.shared.cacheFirstOpenFailCount()
                }
            }
        } else {
            CacheUtil.shared.appendCache(RequestCache(id, key: key, eventKey: eventKey, req: request))
